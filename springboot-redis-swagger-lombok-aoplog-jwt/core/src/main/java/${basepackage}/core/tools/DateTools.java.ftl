...
//        g.setGregorianChange(stringToDate("2017-08-19 00:00:00"));
    ...
    /**
     * 时间计算，获得指定类型，时间变动后的日期时间
     *
     * @param baseTime 基准时间
     * @param dateNum  时间变动值
     * @param dateType 时间变动类型
     * @return
     */
    public static Date getSomeOneDate(Date baseTime, int dateNum, DateType dateType) {
        Calendar calendar = Calendar.getInstance();
        ...
        switch (dateType.val) {
            case 0: {
                calendar.set(year + dateNum, month, date, hour, minute, second);
                break;
            }
            case 1: {
                calendar.set(year, month + dateNum, date, hour, minute, second);
                break;
            }
            case 2: {
                calendar.set(year, month, date + dateNum, hour, minute, second);
                break;
            }
            case 3: {
                calendar.set(year, month, date, hour + dateNum, minute, second);
                break;
            }
            case 4: {
                calendar.set(year, month, date, hour, minute + dateNum, second);
                break;
            }
            case 5: {
                calendar.set(year, month, date, hour, minute, second + dateNum);
                break;
            }
            default: {
    ...
    public enum DateType {
        ...
        //通过值获得枚举对象
        public static DateType getEnum(String name) {
