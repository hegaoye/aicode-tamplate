...
import com.ponddy.core.enums.FormatEnum;
...
public class BaseException extends RuntimeException implements Serializable {
    ...
    public BaseException(ExceptionEnums exception) {
        super(BaseExceptionDTO.toString(exception.codeEnum));

        if (log.isDebugEnabled()) {
            log.debug(">>> 业务异常：[{}] >>>", exception.toString());
        }
    }
    ...
    public BaseException(HttpCodeEnum codeEnum, String message) {
        ...
        if (log.isDebugEnabled()) {
            log.debug(">>> 业务异常：[状态码：{}，异常信息：{}] >>>", codeEnum.code, message);
        }
    }

    public BaseException(ExceptionEnums exceptionMessage, String... params) {
        super(BaseExceptionDTO.toString(exceptionMessage.codeEnum, params));
        if (log.isErrorEnabled()) {
            log.error(">>> 系统发生异常，异常类型： [{}] \n 数据：[{}] >>>", exceptionMessage.toString(), JSON.toJSONString(params));
        }
    }
    ...
    public enum ExceptionEnums {
        ...
        param_is_null(HttpCodeEnum.code_52),
